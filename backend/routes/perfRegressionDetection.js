...
// Performance regression detection identifying test-execution degradation
// Feature: perf-regression-detection
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITestGenerationPlatform" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Performance regression detection identifying test-execution degradation.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'perf-regression-detection', payload, result);
    res.json({ success: true, feature: 'perf-regression-detection', ...result });
  } catch (err) {
    console.error('perf-regression-detection analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITestGenerationPlatform". Return strict JSON only.',
      `Feature: Performance regression detection identifying test-execution degradation. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'perf-regression-detection', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'perf-regression-detection', title: 'Performance regression detection identifying test-execution degradation', project: 'AITestGenerationPlatform' });
});
