...
// VCS webhook integration auto-running suites on PR open
// Feature: vcs-webhook-integration
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: VCS webhook integration auto-running suites on PR open.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'vcs-webhook-integration', payload, result);
    res.json({ success: true, feature: 'vcs-webhook-integration', ...result });
  } catch (err) {
    console.error('vcs-webhook-integration analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: VCS webhook integration auto-running suites on PR open. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'vcs-webhook-integration', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'vcs-webhook-integration', title: 'VCS webhook integration auto-running suites on PR open', project: 'AITestGenerationPlatform' });
});
