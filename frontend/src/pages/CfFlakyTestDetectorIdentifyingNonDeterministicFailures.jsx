...
export default function CfFlakyTestDetectorIdentifyingNonDeterministicFailures() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-flaky-test-detector-identifying-non-deterministic-failures/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-flaky-test-detector-identifying-non-deterministic-failures', project: 'AITestGenerationPlatform' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-flaky-test-detector-identifying-non-deterministic-failures/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Flaky test detector identifying non-deterministic failures</h1>
