...
export default function CfPerformanceRegressionDetectionIdentifyingTestExecutionDegradation() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-performance-regression-detection-identifying-test-execution-degradation/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-performance-regression-detection-identifying-test-execution-degradation', project: 'AITestGenerationPlatform' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-performance-regression-detection-identifying-test-execution-degradation/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Performance regression detection identifying test-execution degradation</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITestGenerationPlatform</p>
      </div>
