...
export default function CfVcsWebhookIntegrationAutoRunningSuitesOn() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-vcs-webhook-integration-auto-running-suites-on-pr-open/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-vcs-webhook-integration-auto-running-suites-on-pr-open', project: 'AITestGenerationPlatform' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-vcs-webhook-integration-auto-running-suites-on-pr-open/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">VCS webhook integration auto-running suites on PR open</h1>
