...
export default function GapCriticalGapNoAiDrivenTestGeneration() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-gap-no-ai-driven-test-generation-despite-domain/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-critical-gap-no-ai-driven-test-generation-despite-domain', project: 'AITestGenerationPlatform' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-gap-no-ai-driven-test-generation-despite-domain/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Critical gap: No AI-driven test generation despite domain centrality</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AITestGenerationPlatform</p>
      </div>
